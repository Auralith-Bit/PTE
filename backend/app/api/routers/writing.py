...
from app.schemas.enums import QuestionDifficulty, WritingType
...
def list_questions(
    ...
    questions = get_questions(db, "writing", t, d, limit, offset, shuffle)
    total = count_questions(db, "writing", t, d)
...
def get_question(question_id: int, db: Session = Depends(get_db)) -> QuestionOut:
    ...
    if question is None or question.category != "writing":
